...
\begin{itemize}
    ...
    \item $r^{\mathrm{task}}_i(t)$ --- TEBC task response for neuron $i$, drawn from type $k \in \{1,\dots,8\}$ (additive/dependent) or $k \in \{1,\dots,4\}$ (place-dependent)
    \item $\beta_i$ --- per-neuron balance parameter
    \item $b_i \equiv \tilde{r}^{\mathrm{place}}_i(t)$ --- live place rate, used as baseline in dependent models
...
\begin{tabular}{llll}
    ...
    \hline
    Additive         & $0\,\text{Hz}$ & $1\,\text{Hz}$  & biologically low; task constants tuned to match \\
    Dependent        & $0\,\text{Hz}$ & $12\,\text{Hz}$ & physiologically realistic \\
...
% TODO: validate the units of the type 1--8 response functions in the additive model
...
\noindent
Each profile $R_k(t, b)$ has the same functional form as in the additive model
...
The following profiles are shared by the additive and dependent models.
In the additive model the magnitude parameters are fixed constants (given as defaults below).
In the dependent model they are expressed as multiples of the live baseline
$b_i = \tilde{r}^{\mathrm{place}}_i(t)$ (see the dependent model table above).
