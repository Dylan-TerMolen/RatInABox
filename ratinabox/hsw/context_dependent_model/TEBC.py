...
cell functionalities. This class takes in two balance distributions. One that is used when the agent is in trial and one when the agent is not in a trial
...
class TEBC(PlaceCells):
    ...
    def __init__(self, agent, N, balance_distribution, task_balance_distribution, responsive_distribution, percent_place_cells, tebc_responsive_neurons=None, cell_types=None):
        place_cells_params = {
            "n": N, 
        ...
        self.task_balance_distribution = task_balance_distribution
    ...
    def update(self):
        ...
        balance_distribution = self.task_balance_distribution if self.agent.in_trial else self.balance_distribution

        self.firingrate = (balance_distribution * task_firing_rates) + (self.firingrate * (1 - balance_distribution))
