...
def _normalized(shape_func):
    vals = np.array([shape_func(t) for t in _t_ref])
    f_min, f_max = vals.min(), vals.max()
    return lambda t, peak_fr: (shape_func(t) - f_min) / (f_max - f_min) * peak_fr


response_profiles = {
    1: {'response_func': _normalized(lambda t: bimodal_response(t, [.15, .4], [0.3, .85], [0.1, 0.1], .07, CS_duration, US_duration))},
    2: {'response_func': _normalized(lambda t: gaussian_peak2(t, .22, US_start_time, 0.15, US_duration, .07))},
    3: {'response_func': _normalized(lambda t: linear_decay_response(t, peak_time=0, mid_time=.2, end_time=.85, baseline=.06, first_decrement=0.02, second_decrement=0.01))},
    4: {'response_func': _normalized(lambda t: cell_type_4_response(t, baseline=.075, cs_peak=0.03, us_peak=0.08, cs_sd=0.1, us_sd=0.2, cs_decay_start=.25, us_peak_time=.75, us_duration=0.1))},
    5: {'response_func': _normalized(lambda t: cell_type_5_response(t, baseline=.035, us_peak=0.11, us_duration=US_duration))},
    6: {'response_func': lambda t, peak_fr: noisy_uniform_response(t, peak_fr * 0.5)},
    7: {'response_func': _normalized(lambda t: bimodal_response2(t, [.07, .09], [0.3, US_start_time+.05], [0.08, 0.15], .042, cs_duration=.25, us_duration=.1))},
    8: {'response_func': _normalized(lambda t: type8decay(t, .065))},
}
