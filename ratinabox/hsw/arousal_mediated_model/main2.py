...
from ratinabox.hsw.arousal_mediated_model.assign_tebc_types_and_responsiveness import assign_tebc_types_and_responsiveness
from ratinabox.hsw.arousal_mediated_model.simulate_agent import simulate_agent
...
save_directory = config.get_save_directory(model_name='arousal_mediated')
config.setup_ratinabox_figure_directory(save_directory)

# Construct the filename
current_date = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
results_file_base = os.path.join(save_directory, f"{current_date}:ARM_results-response-{args.responsive_values}-{args.responsive_type}-PCs-{args.percent_place_cells}")
...
for responsive_val, percent_place_cell, holdover in itertools.product(responsive_values, percent_place_cells, holdovers):
    ...
    for i in range(num_iters):
        ...
        filename_envA = f"ARM_response_envA_responsive_{responsive_val}_{args.responsive_type}_perPCs_{percent_place_cell}_holdovers_{args.holdover_type}.npy"
        filename_envB = f"ARM_response_envB_responsive_{responsive_val}_{args.responsive_type}_perPCs_{percent_place_cell}_holdovers_{args.holdover_type}.npy"
