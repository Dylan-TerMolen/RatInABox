...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --array=0-8  # one task per CEBRA grid combination (3 learning_rate x 3 output_dimension)
#SBATCH --mem=8GB
#SBATCH --time=1:00:00
...
LEARNING_RATES=(1e-4 3e-4 8.6e-4)
...
DIM=${OUTPUT_DIMENSIONS[$IDX]}

echo "Task ${SLURM_ARRAY_TASK_ID}: cebra_learning_rate=${LR} cebra_output_dimension=${DIM}"
