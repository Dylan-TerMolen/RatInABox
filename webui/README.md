...
(Northwestern's HPC cluster) for `ratinabox/hsw`'s simulations, without
hand-editing `.sh` files or babysitting `squeue` over SSH.

Scoped to this repo only -- hannahs-cebras is a decoding utility library
that `hsw/main.py` imports (installed as the `hannahs_cebras` pip package on
Quest), not something jobs get submitted to directly, so it's out of scope
here entirely.

## What it does

- **Script picker** — auto-detects `ratinabox/hsw/main.py`'s CLI params via
  real `argparse` introspection and renders a form instead of you typing a
  command from memory.
- **SLURM generator** — turns the filled-in form into a `.sh` script matching
  the account/partition/GPU conventions already used by hand in
  `hsw/*/SLURM_*.sh`. Any param can be marked "grid" (comma-separated
  values) to sweep it as a `--array` job, same row-major mapping pattern as
  `SLURM_place_dependent.sh`.
- **Upload existing `.sh`** — for a script not covered by the generator, upload
...
- **Sync** — pulls the results dir down from Quest via `rsync`, only when
  you click "Sync now".
...
1. **Edit `webui/config.yaml`** — every `CHANGE_ME` needs a real value:
   ...
   - `repos.ratinabox.local_path` / `remote_path` — where the repo lives on
...
2. **Install dependencies** into the same conda env you run the analysis
   scripts from (script introspection imports `args_parser.py` directly):
...
|---|---|---|
| Upload to Quest              | `POST /jobs/{id}/upload`  | one ssh call: `cat > <path>` (script piped over stdin). Falls back to a one-time `mkdir -p <dir>` + retry only if the write fails (e.g. the very first upload) |
...
state (upload takes a second one-off round trip only the first time
`webui_slurm/` doesn't exist yet), nothing else in the app shells out
anywhere. `quest.check_connection()` (a plain `ssh ... whoami`) exists as a
smoke test but isn't wired to a button yet.
...
- Just `ratinabox/hsw/main.py` for now -- hannahs-cebras is out of scope
  (see the note at the top). Use "Upload existing .sh" for any script this
  doesn't cover.
